...
axios.get('https://data.splitgraph.com/splitgraph/oxcovid19/latest/-/rest/administrative_division?and=(countrycode.eq.SWE,adm_level.eq.1)&select=country%2Cadm_area_1%2Cgeometry')
    .then(response => {
        console.log('ADMINISTRATIVE_DATA')
...
function areaExists (area) {
    let arr = geojson.features
    return arr.some(function (el) {
        return el.admin_level_1 === area;
    });
}

axios.get('https://data.splitgraph.com/splitgraph/oxcovid19/latest/-/rest/epidemiology?countrycode=eq.SWE&adm_area_1=eq.Blekinge&date=eq.2021-03-10&select=adm_area_1%2Cdate%2Csource%2Cdead')
    .then(response => {
        console.log('EPIDEMIOLOGY DATA')
        console.log(response.data[0]);
        let apiData = response.data;
        let inData = response.data[0].admin_level_1;
        console.log('Hej', inData);
        let test = areaExists(inData);
        console.log('EPIDEMIOLOGY TEST DATA');
        console.log(test);
    })
    .catch(error => console.error(error));
